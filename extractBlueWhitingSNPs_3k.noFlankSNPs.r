...
high_variance_freq <- read.table("INTERSECT_all_vs_ICE_E_ICE_W_MED_N_SPA_POR_ALG_SE_IRE_SPA_Cor.nInfTotOver150.unkn_asZero.fixCHRnames.FILT.no.SNPs.37bp.forMatsscript.bed", stringsAsFactors=F, header = T)
...
tSNPs <- stepwise_select(SNP_freq_df = high_variance_freq, target_pops = 3:dim(high_variance_freq)[2], n_steps = 20, n_iter = 100, n_snps =3000, hm_file = "SNP_selection_test_blue_whiting_3.NoFlankSNPs.hm.pdf", scaffold_re = "NC_")

write.table(tSNPs$SNP_set$picked_SNPs, file = "selected_SNPs_bw_3k.NoFlankSNPs.txt", row.names = T, quote = F, sep = "\t")
